...
# Here we develop a one-dimensional numerical model of diffusion.
# It assumes a constant diffusivity and uses a regular grid. It has a step function as an initial profile. It has fixed boundary conditions.
...
# Here $z$ is elevation, $t$ is time, $x$ is the spatial dimension, and $D$ is the hillslope diffusivity.

# This is the discretized version of the diffusion model we'll solve with our model:
...
# We'll use two libraries, NumPy and Matplotlib that aren't a part of the core Python distribution. I must switch to the CSDMS kernal to use these libraries.
...
# We'll use array operation on the variable $z$. 
